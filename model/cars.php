<?php
    ...
    function get_cars_by_ref($ref)
    {
        ...
        try
        {
            $statement = $db->prepare('SELECT * FROM cars WHERE ref = :ref ORDER BY ref');
    ...
    function update_cars($ref, $make, $model, $man_year, $mileage, $_location, $price, $colour, $transmission, $car_description)
    {
        global $db;
        $query = 'UPDATE cars 
                  SET ref = :ref, make = :make, model = :model, man_year = :man_year, mileage = :mileage, _location = :_location, price = :price, colour = :colour, transmission = :transmission, car_description = :car_description
                  WHERE ref = :ref';
        try
        {
            $statement = $db->prepare($query);
